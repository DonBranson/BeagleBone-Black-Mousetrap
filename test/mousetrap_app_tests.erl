...
fixture_startup_test() ->
  ...
  meck:expect(application, info, 0, [{loaded,[{mousetrap, "", "0.0.0"}]}, nil, nil, nil, nil, nil ]).

start_sends_notification_test() ->
  mousetrap_app:start("", ""),
  ?assert(meck:called(notification_library, notify, ["@channel Mousetrap 0.0.0 starting"])).
...
stop_sends_notification_test() ->
  ...
  meck:expect(mousetrap_sup, stop, 0, {ok, pid}),
  mousetrap_app:stop(state),
  ?assert(meck:called(notification_library, notify, ["@channel Mousetrap stopping"])).
